...
* ~~Add a "Projects" feature that actually scopes the tasks to a project. Currently you can create projects, but only the first one actually has Sprints or Tasks associated with it.~~ Done, it didn't have much trouble with this.
...
* ~~Provide the ability to change which project you are viewing. Again, everything is sort of global scope at the moment.~~ Done, it had some trouble with this but a couple iterations feeding it it's errors got us there. Also had to fix some styling issues over a couple of tries.
* ~~Security - There isn't any at the moment. I want to see it provide OAuth with a common provider like Google to start with, then maybe we can look at maintaining separate credentials with either email verification or 2FA (probably the former, don't really want to spend money on this)~~ This one was kind of painful and I had to manually intervene to solve one issue it got stuck on (Hibernate problem). Still got most of it done but was very adamant about doing security a certain way that may not have been optimal. Username/password still to be done.
